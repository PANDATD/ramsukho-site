...
html_dir="/home/pandatd/Documents/public_html/"
...
for file in "$html_dir"/*.html; do
    ...
    if [ -f "$file" ]; then
        # Loop through each entry in the new_alt_text_mapping
        for image in "${!new_alt_text_mapping[@]}"; do
            new_alt_text="${new_alt_text_mapping[$image]}"
            # Use sed to add or replace the alt attribute in <img> tags for each image
            sed -i -E "s|(<img[^>]*src=\"[^\"]*${image}\"[^>]*)(alt=\"[^\"]*\"|>|)>|\1 alt=\"${new_alt_text}\"|g" "$file"
        done
        echo "Processed $file"
    fi
done

echo "All specified alt tags updated or added."
